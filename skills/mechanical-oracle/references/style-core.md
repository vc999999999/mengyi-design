...
强调色用法:电蓝 / 朱红 / 品红 / 镉黄 都是**自由强调 / 装饰色**,按画面需要取用,**不绑定栏目语义、不做分类标签**。惯例上镉黄作「高亮 / 变异字 / 按钮」。
...
## 4 · 三个固定锚点(每个版面/每张图都要有)

1. **刊头条** — `电子梦呓 / DIGITAL REVERIE` + 期号(Space Mono 小字,色 `#7a6f59`)。
2. **变异标题带** — 900 大标题,**只让一个字变异**(`transform:scaleY(1.4)` 拉长 / 换镉黄或品红 / 套 `filter:url(#warp)`)。
3. **孔洞/圆窗 = 插画位** — 可爱主角从圆窗/开口探出(粗黑边图框,圆角更萌),或整版满铺,二选一。
